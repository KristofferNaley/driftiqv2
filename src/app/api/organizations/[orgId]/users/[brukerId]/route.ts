...
export const PUT = orgRute<P>({
  ...
  handler: async ({ db, orgId, bruker, params, req }) =>
    endreMedlemskap(db, orgId, params.brukerId, await lesKropp(req, medlemEndring), aktorFor(bruker)),
});
...
export const DELETE = orgRute<P>({
  nivaa: "admin",
  status: 204,
  handler: ({ db, orgId, bruker, params }) => fjernFraOrg(db, orgId, params.brukerId, aktorFor(bruker)),
});
